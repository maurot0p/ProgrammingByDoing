...
public class FunctionCallAlphabet
{
	public static void main( String[] args )
	{
		...
		f("florida");// ???? f( ???? );    // displays the word you pass it beginning with "F"
		...
		n("norton");// ???? n( ???? );    // displays the word you pass it beginning with "N"
		...
		t("torero");// ???? t( ???? );    // displays the word you pass it beginning with "T"
		u();// ???? u( ???? );    // returns the word to be displayed
		v(1);// ???? v( ???? );    // tell it how many times to display the word (1)
		w(63);// ???? w( ???? );    // pass it any integer and it will display a word beginning with W 
		x(1);// ???? x( ???? );    // returns a different word depending on what you pass it (1-2)
		y();// ???? y( ???? );    // displays a word beginning with Y
		z(true,true);// ???? z( ???? );    // returns a different word depending on which two boolean values you pass it
		System.out.println();
	}
